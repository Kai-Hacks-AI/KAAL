...
Check with `scripts/check.ts <skills-dir>`: every directory in it is a skill, and every way a skill breaks the standard or differs from what its init generates is reported. Checking runs each skill's init with tsx in a scratch copy of the skill without its `SKILL.md`, never over the skill. It bounds a stuck init but is not a sandbox: init is the skill's own code, run as the caller, and a failed init is reported without its output, so run it to see why. Lengths count characters, not UTF-16 units. From code, `checkSkills` in `scripts/skills.ts` returns the same errors, and `standardErrors` and `birthErrors` check one skill against each rule. Which skills to keep and when to check them is the using system's decision; this skill only checks.
