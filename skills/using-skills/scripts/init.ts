...
Born from init: running the skill's \`scripts/init.ts\` (\`tsx scripts/init.ts\`) writes the skill's \`SKILL.md\` next to the scripts. \`SKILL.md\` is never edited by hand: change init and run it. Init is self-contained: it uses only Node's built-in modules and its own skill's files. A skill is born correctly when its committed \`SKILL.md\` is exactly the bytes its init generates, so the using system must keep \`SKILL.md\` byte-exact wherever it is checked out and never reformat it.
