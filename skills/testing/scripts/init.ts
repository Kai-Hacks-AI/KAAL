...
Decide what testing preserves. A test strategy is the lasting set of decisions every plan follows: what testing must preserve, what a change must prove, and how what one change proves becomes what later changes must preserve. It outlives its plans and shapes many of them: a regression plan for what is already accepted, and a plan for each change in progress, which must show that the regression still holds and prove what the change adds. The same cases and suites can serve both. When a change is accepted, the commitments its plan proved, under the conditions it proved them, join the regression the next change must keep. Commitments the change leaves in place are judged by checks it cannot alter, and where no such check exists yet, the plan names the gap; a commitment the change replaces or withdraws is named in its plan, because the checks that protected it would otherwise reject the change. A replaced commitment's successor is proven by the change's own checks; a withdrawn one simply leaves the regression, visibly, when the change is accepted.

Decide how testing learns. The strategy also says how coverage is chosen, such as proving a claim in every environment the tested thing supports wherever the environment can change the result, and saying where it is not proven. It says how failures are learned from: a failure found by review or use gets proof that fails before the fix and passes after it, wherever it was missing: a new case for a claim not yet stated, new data for a claim already stated, a proof that covers more of its claim, or a condition the plan did not yet name. When the same kind of failure keeps being found, the fix changes the mechanism that lets it happen instead of only the instance found, while that proof still covers the instance.
