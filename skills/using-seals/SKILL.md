...
Seal with `scripts/seal.ts <root> <chain> <unit>...`: every open unit of the chain is sealed, oldest first, and the chain's head moves to the newest seal. Sealing refuses a chain whose seals are already broken, an empty unit, a unit holding anything but regular files and directories (symlinks, FIFOs, sockets, devices), and a unit inside or containing a unit already sealed by any chain, and writes nothing when it refuses. A fully sealed chain is left as it is.
