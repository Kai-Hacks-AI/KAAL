...
Seal with \`scripts/seal.ts <root> <chain> <unit>...\`: every open unit of the chain is sealed, oldest first, and the chain's head moves to the newest seal. Sealing refuses a chain whose seals are already broken, an empty unit, a unit holding anything but regular files and directories (symlinks, FIFOs, sockets, devices), and a unit inside or containing a unit already sealed by any chain or named as another chain's head, and writes nothing when it refuses. Every chain's head shares \`seals.json\`, so sealing holds \`seals.json.lock\` at the root while it runs and refuses to start while another sealing holds it; a lock left behind by a sealing that crashed is removed by hand; if a write fails halfway, it removes the seals it wrote, leaving the chain as it was. A fully sealed chain is left as it is.
