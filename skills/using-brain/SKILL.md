...
A node has a stable `name`. Its path is its identity. Lineages and slugs are path components, so they must be portable: lowercase kebab-case (`a-z`, `0-9`, single hyphens) and never a Windows reserved device name such as `con` or `nul`. The same name then means the same file on every platform. Nodes with the same name may be born later as understanding changes; earlier nodes are not changed.
