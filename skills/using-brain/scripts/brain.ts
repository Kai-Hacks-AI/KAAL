...
export function parseNode(file: string): Frontmatter {
  ...
  if (data.edges !== undefined) {
    if (!Array.isArray(data.edges)) throw new Error(`${file}: edges must be an array`);
    for (const edge of data.edges) {
      if (!isRecord(edge) || typeof edge.relation !== "string" || typeof edge.to !== "string") {
        throw new Error(`${file}: each edge needs string relation and to`);
      }
    }
  }
...
export function relativeIdentity(root: string, file: string): string {
...
export function nodeIndex(root = ROOT): Map<string, string> {
  return new Map(nodeFiles(root).map((f) => [relativeIdentity(root, f), f]));
}
...
export function edgeErrors(
  ...
  for (const edge of edges) {
    for (const [kind, id] of [
      ...
      if (!file) {
        errors.push(`${self}: missing ${kind} ${id}`);
        continue;
      }
      try {
